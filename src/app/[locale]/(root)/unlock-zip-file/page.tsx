...
const SLUG = 'unlock-zip-file';
const config = ZIP_TOOL_CONFIGS[SLUG];

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  const title = `${config.toolName} - Free Online Archive Tool | ConvertAllNow`;
...
const Page = async ({ params }: { params: Promise<{ locale: string }> }) => {
  ...
  const schemaData = createToolSchema({
    name: `${config.toolName} - Free Online Archive Tool`,
...
const howToSteps = [
  ...
  const breadcrumbItems = [
    { name: 'Home', href: '/' },
    { name: 'Unlock Zip File', href: '/unlock-zip-file' },
  ];
  const relatedTools = RELATED_TOOLS['unlock-zip-file'] || [];
  ...
  return (
    <>
      ...
      <FAQSchema faqs={faqs} />
      <WebPageSchema title={title} description={description} path="/unlock-zip-file" locale={locale} breadcrumb={breadcrumbItems} />
